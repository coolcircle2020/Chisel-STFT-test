...
class Hanning extends Module
  ...
  val cosine_value   = Reg(new MyFixedPoint)
  ...
  when(state === s_idle){
    ...
    cosine_value :=  cosinesbank.read(cosinesbank_rcnt)
    when(cosinesbank_rcnt === 513.U){cosinesbank_rcnt:= 0.U} //halt
  }.elsewhen(state === s_trans){
    cosinesbank_rcnt := cosinesbank_rcnt + 1.U //cnt++
    cosine_value := cosinesbank.read(cosinesbank_rcnt) //read cosine value
    hanning_signal.value := io.signal.bits.value * cosine_value.value  //compute hanning window
